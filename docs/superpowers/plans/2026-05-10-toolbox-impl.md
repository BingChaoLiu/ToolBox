...
```bash
python run.py hello --name ToolBox
```

Expected:
```
Hello ToolBox!
{'greeting': 'Hello ToolBox'}
```

- [ ] **Step 3: 验证无脚本时报错**

```bash
python run.py nonexistent
```

Expected: `脚本不存在: D:\Document\Project\ToolBox\scripts\nonexistent.py`

- [ ] **Step 4: 验证无 main() 时报错**

```bash
cat > scripts/no_main.py << 'EOF'
print("no main function")
EOF

python run.py no_main
```

Expected: `脚本 'no_main' 没有 main() 函数`

```bash
rm scripts/no_main.py
```

- [ ] **Step 5: 提交**

```bash
git add run.py scripts/hello.py
git commit -m "feat: 脚本独立运行器，支持 --参数名 值 命名传参"
```

---

### Task 18: main.py — 入口与 --init-script
...
def main():
    ...
    if args.init_script:
        try:
            ...
            pass  # init-script 不强制要求 config.yaml 存在
...
- [ ] **Step 2: 验证 --init-script**

```bash
python main.py --init-script scripts/hello.py
```

Expected:
```
已生成清单模板: scripts\hello.yaml
请编辑清单，补充参数类型、剪贴板、输出字段等信息。
```
...
- [ ] **Step 3: 验证 --init-script 重复运行提示已存在**

```bash
python main.py --init-script scripts/hello.py
```

Expected: `清单文件已存在: scripts\hello.yaml`
...
验证 TUI 启动正常，左侧显示 hello 脚本。

- [ ] **Step 5: 提交**

```bash
git add main.py
git commit -m "feat: 入口文件，支持 --frontend 和 --init-script"
```

---

### Task 19: config.example.yaml + 示例脚本

**Files:**
- Create: `config.example.yaml`
- Create: `scripts/addr2line.py` + `scripts/addr2line.yaml`
- Create: `scripts/clean_build.py`
- Create: `pipelines/crash_analysis.yaml`
...
addr2line:
  tool_path: "addr2line"  # 或 NDK 完整路径
...
email:
  ...
  auth_code: "your_auth_code"  # QQ 邮箱 IMAP 授权码
  filter:
...
```yaml
# scripts/addr2line.yaml
name: 崩溃地址分析
description: 解析 native 崩溃地址对应的源码位置
category: 调试
...
- [ ] **Step 3: 创建 clean_build.py（Tier 1 示例，无清单）**
...
- [ ] **Step 4: 创建 crash_analysis 流水线示例**

```yaml
# pipelines/crash_analysis.yaml
name: 崩溃分析流水线
description: 从剪贴板读取崩溃地址，分析源码位置
...
steps:
  - id: analyze
    ...
    mapping:
      address: "${choice}"
      so_file: "${config:addr2line.default_so}"
...
- [ ] **Step 5: 运行 --init-script 验证自动生成**

```bash
python main.py --init-script scripts/clean_build.py
```

验证生成 `scripts/clean_build.yaml`。

- [ ] **Step 6: 运行 run.py 验证独立执行**

```bash
python run.py clean_build
```

Expected: `清理完成`（如果没有配置文件会报错，这是预期行为）

- [ ] **Step 7: 提交**

```bash
git add config.example.yaml scripts/addr2line.py scripts/addr2line.yaml scripts/clean_build.py pipelines/crash_analysis.yaml
git commit -m "feat: 配置模板与示例脚本（addr2line、clean_build、crash_analysis 流水线）"
```
...
编辑 `config.yaml`，将 `build.project_path` 设为当前目录用于测试：
...
- [ ] **Step 3: 验证 run.py 独立运行**

```bash
python run.py hello --name "Integration Test"
```

Expected: `Hello Integration Test!`

- [ ] **Step 4: 验证 --init-script 生成清单**

```bash
python main.py --init-script scripts/clean_build.py
cat scripts/clean_build.yaml
```

Expected: 输出包含 name、category、params、outputs 字段
...
验证项：
1. TUI 正常启动，Header 显示 "ToolBox"
2. 左侧菜单按分类显示脚本：调试（崩溃地址分析）、未分类（hello、clean_build）
3. 流水线区域显示"崩溃分析流水线"
4. 点击 hello → 右侧显示参数表单
5. 按 F9 → 输出面板显示 "Hello World!" 和执行完成
6. 按 F11 → 全屏输出
7. 按 Ctrl+S → 导出日志文件
8. 按 F5 → 刷新菜单
9. 按 Ctrl+C 退出
...
```bash
# 独立运行
python run.py date_check

# 生成清单
python main.py --init-script scripts/date_check.py

# 启动 TUI，F5 刷新菜单后应该能看到新脚本
python main.py
```
...
```bash
# 清理测试生成的临时文件
rm -f scripts/clean_build.yaml scripts/date_check.py toolbox_output_*.log config.yaml
git add -A
